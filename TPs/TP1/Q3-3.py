...
with open("formal-tst.NE.key.04oct95_sample.txt", "r") as f:
    text = f.read()
...
with open("formal-tst.NE.key.04oct95_sample.txt.ne.nltk", "w") as f:
...
# Écrire les entités nommées dans un fichier
with open("formal-tst.NE.key.04oct95_sample.personnalise.txt.ne.nltk", "w") as f:
    for chunk in named_entities:
        if hasattr(chunk, 'label'):
